...
TEST_START(1, "nn_create")
{
    ...
    {
        {.type = NEURAL, .activ_func = RELU, .max = 0.2, .min=0.1, .size = 10},
        {.type = DROP, .drop_rate = 40},
        {.type = NEURAL, .activ_func = NO_FUNC, .max = 0.2, .min=01, .size = 20}
    };
    nn_array_t* n = nn_create(100, 15, 3, initializer);

    TEST_SIZE(n->layers[0].delta,   10, 15)
    TEST_SIZE(n->layers[0].out,     10, 15)
    TEST_IF(n->layers[0].type != NEURAL)
    ...
    TEST_IF(n->layers[2].type != NEURAL)

...
static inline mx_size
count_reshaped_input_y(const mx_size x_krnl_count, const mx_size y_krnl_count, const mx_size batch)
{
...
conv_forwarding(struct nl_t *self, const mx_t *input)
{
	...
	fill_mx_by_idx_mx(input, conv_data->image_sections, conv_data->im2col_idxs);
...
conv_backwarding(const nn_t *nn, const nn_size idx, const mx_t *prev_out, optimizer_t opt)
{
	...
	if (conv_data->act_func.func_cell != NULL)
		mx_hadam_lambda(
			nn->delta[self->cache_idx],
			*self->out, 
			conv_data->act_func.func_cell);
	mx_set_size(nn->temp, self->weights->x, self->weights->y);
	mx_mp(*nn->delta[self->cache_idx], *conv_data->image_sections, nn->temp, A);
	if (idx) {
	...
	const mx_type max)
{
	...
	const mx_size x_krnl_count = count_x_kernels(input_size, kernel_size, stride);
	const mx_size y_krnl_count = count_y_kernels(input_size, kernel_size, stride);
	...
	if ((curr->out = mx_create(output_x, output_y)) == NULL || data == NULL
	   || conv_data == NULL)
		goto conv_err_exit;
	
	conv_data->act_func = act_func;

	if ((conv_data->im2col_idxs = get_im2col_idxs(kernel_size, input_size, stride, nn->batch_len)) == NULL)
		goto conv_err_exit;
	
	if ((conv_data->image_sections = mx_create(
	       count_reshaped_input_x(input_size, kernel_size),
	       count_reshaped_input_y(x_krnl_count, y_krnl_count, nn->batch_len))) == NULL)
	       goto conv_err_exit;
	
	if ((curr->weights = mx_create(kernel_size.x * kernel_size.y * input_size.z, kernel_size.z)) == NULL
	   || mx_recreate_if_too_small(nn->temp, curr->weights->x, curr->weights->y))
	   	goto conv_err_exit;
	...
	data->size = (img_size_t) {.x = x_krnl_count, .y = y_krnl_count, .z = kernel_size.z};

	...
	const mx_type max)
{
	...
	const mx_size in = (nn->len) ? nn->layers[nn->len - 1].out->x :
				       nn->in_len;
 ...
 */
	if (mx_recreate_if_too_small(nn->delta[curr_cache], neurons,
				     nn->batch_len))
		goto dense_err_exit;
 ...
 */
	...
	if ((curr->out = mx_create(neurons, nn->batch_len)) == NULL ||
	    data == NULL)
		goto dense_err_exit;
